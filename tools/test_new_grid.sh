...
TMP_MANIFEST="target/new_grid_cases.tsv"
...
echo "[2/3] collect new_grid subset manifest"
{
  printf 'case_id\tfixture_id\tfixture_path\tprepare_cmd\twgrib2_cmd\tmbt_cmd\tcompare_mode\tnote\n'
  rg '_new_grid_' fixtures/wgrib2_snapshots/grid/manifest_v2.tsv
} > "$TMP_MANIFEST"
printf '  cases=%s\n' "$(($(wc -l < "$TMP_MANIFEST") - 1))"

echo "[3/3] compare new_grid subset"
